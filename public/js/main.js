...
const item = document.querySelectorAll('.item span') // creating a variable and assigning it to a selection of span tags inside of a parent that has the class of 'item'
const itemCompleted = document.querySelectorAll('.item span.completed') // creating a variable and assigning it to a selection of spans with the class of 'completed' inside of a parent with a class of 'item'
...
Array.from(item).forEach((element)=>{ // Creating an array from our collection and starting a loop
...
async function deleteItem(){ // declare an asynchronous function
    const itemText = this.parentNode.childNodes[1].innerText // looks inside of the list item and grabs only the inner text within the list span (comments may interfere with child nodes)
...
async function markUnComplete(){ // declare an asynchronous function
    ...
    }catch(err){ // If an error occurs pass it to the catch block
        console.log(err) // log the error to the console
    } // close the catch block
} // end the function