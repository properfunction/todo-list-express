...
const MongoClient = require('mongodb').MongoClient // making it possible to use methods associated with MongoClient and talk to our DB
...
app.get('/',async (request, response)=>{ // Starts a GET method when the root route is passed in, sets up req and res parameters
    ...
    const itemsLeft = await db.collection('todos').countDocuments({completed: false}) // sets the variable and awaits a count of uncompleted items to later display in EJS
...
app.put('/markUnComplete', (request, response) => { // starts a PUT method when the markUnComplete route is passed in
    db.collection('todos').updateOne({thing: request.body.itemFromJS},{ //look in the db for one item matching the name of the item passed in from the main.js file that was clicked on 
        ...
        upsert: false // prevents creation if item does not already exist
    })
    .then(result => { // starting a then if update was succesful
        console.log('Marked Complete') // logging succesful completion
        response.json('Marked Complete') // sending a response back to the sender
    })
    .catch(error => console.error(error)) // catching errors
...
app.delete('/deleteItem', (request, response) => { // starts a delete when the delete route is passed
    db.collection('todos').deleteOne({thing: request.body.itemFromJS}) // look inside the todos collection for the ONE item hat has a matching name from our JS file
    .then(result => { // starts a then if delete was succesful
        console.log('Todo Deleted') // logs succesful completion
...
app.listen(process.env.PORT || PORT, ()=>{ // setting up which port we will be listening on - either from .env file or variable we set
